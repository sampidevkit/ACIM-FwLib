...
#define Ts          50.0 // Time lay mau 50us
...
double alpha=0;
double v_star;
float Ta, Tb, T0, duty1, duty2, duty3;
float v_alpha;
float v_beta;
static int n=0;
int t=0;
...
void Clarke_Transform(float u, float v, float* a, float* b)
{
    *a=u;
    *b=(1.0/SQRT_3)*u+(2.0/SQRT_3)*v;
}
...
// Normalize Angle (from 0 - pi to 0 - 2*pi)

float _normalizeAngle(float angle)
{
    float a=fmod(angle, PI*2);
    return a>=0?a:(a+PI*2);
}

void SVPWM(int t)
{
    Clarke_Transform(VU, VV, &v_alpha, &v_beta);

    alpha=atan2(v_beta, v_alpha);
    alpha=_normalizeAngle(alpha);
    v_star=sqrt(pow(v_alpha, 2)+pow(v_beta, 2))*17.89785834;

    // sector
    n=((int) (alpha/(PI/3.0)))%6+1;

    // SVPWM: Cac thong so can thiet
    Ta=((sqrt(3)*Ts*v_star)/vdc)*sin((PI*n)/3-alpha);
    Tb=((sqrt(3)*Ts*v_star)/vdc)*(-cos(alpha)*sin((n-1)*PI/3.0)+sin(alpha)*cos((n-1)*PI/3.0));
    T0=Ts-Ta-Tb;
